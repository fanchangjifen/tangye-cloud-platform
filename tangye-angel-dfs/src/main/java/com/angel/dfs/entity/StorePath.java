...
public class StorePath {
    ...
    public static StorePath praseFromUrl(String filePath) {
        ...
        if ((pos <= 0) || (pos == groupAndPath.length() - 1)) {
            throw new RuntimeException("解析文件路径错误,有效的路径样式为(group/path) 而当前解析路径为".concat(filePath));
        }
    ...
    private static int getGroupStartPos(String filePath) {
        ...
        if ((pos == -1)) {
            throw new RuntimeException("解析文件路径错误,被解析路径url没有group,当前解析路径为".concat(filePath));
        }
        return pos;
    }
